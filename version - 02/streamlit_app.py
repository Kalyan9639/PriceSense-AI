...
try:
    ...
    # Convert model USD outputs → INR for display
    display_revenue = pred_revenue * INR_TO_USD
    display_profit  = pred_profit  * INR_TO_USD

    # Margin is currency-agnostic (ratio stays the same)
...
try:
    # Direct Function Call — pass INR rate so graph axes display rupee values
    sens_data = backend.get_sensitivity_analysis(payload, inr_rate=INR_TO_USD)
