...
class PriceSense:
    ...
    def predict(self, input_data: PriceInput):
        ...
        #     # st.stop()
        #     import streamlit as st
        #     st.error("Model expected features")
...
#         # 1. Get the 'clean' data from the current user input
#         filled_original = input_data.get_filled_dict()
#         base_price = filled_original["Price"]
        
#         # 2. Create the price range (X-axis)
...
#         for p in prices:
#             # 🛡️ THE FIX: Use the exact keys that PriceInput expects
#             # Map the 'Storage Cost' back to 'cost', etc.
#             temp_input = PriceInput(
#                 price=p,
