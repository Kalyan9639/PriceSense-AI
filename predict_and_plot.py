...
# 🛡️ CRITICAL FIX: HARDCODED COLUMN ORDER
# This forces the dataframe to match the exact order you provided.
MODEL_COLUMNS = [
    "Price",
    "Competitor Price",
    "Discount",
...
class PriceInput(BaseModel):
    ...
    def get_filled_dict(self) -> dict:
        ...
        # Dictionary creation (Order here doesn't matter as much because we reorder later)
        return {
            "Price": self.price,
            "Competitor Price": get_val(self.competitor_price, "Competitor Price", 1000.0),
            "Discount": get_val(self.discount, "Discount", 0.0),
    ...
    def to_dataframe(self) -> pd.DataFrame:
        ...
        # This ensures 'Price' is always column 0, 'Competitor Price' is column 1, etc.
        # If the dataframe has extra columns, they are dropped.
        # If it misses columns, this will raise an error (which is good, we want to know).
        df = df[MODEL_COLUMNS]
...
class PriceSense:
    def __init__(self, model_path: str = MODEL_PATH):
        ...
        try:
            if os.path.exists(model_path):
                ...
                # Show warning on UI if file missing
                import streamlit as st
                st.warning(f"⚠️ Model file NOT found at: {model_path}")
                
        except Exception as e:
            # Show critical error on UI
            import streamlit as st
            st.error(f"❌ CRITICAL ERROR LOADING MODEL: {str(e)}")

    def predict(self, input_data: PriceInput):
        # Dataframe is already sorted correctly by .to_dataframe()
        df = input_data.to_dataframe()
        ...
        if self.model_loaded:
            try:
                ...
                # If prediction fails, show error and stop
                import streamlit as st
                st.error(f"❌ Prediction Error. Model expected features: {MODEL_COLUMNS}. Error details: {e}")
                st.stop()
        else:
            # Fallback logic
            filled = input_data.get_filled_dict()
    ...
    def plot_profit_curve(self, input_data: PriceInput, save_dir="visualization"):
        ...
        fig.add_trace(go.Scatter(
            x=prices, y=revenues, mode='lines', name='Revenue',
            line=dict(color='#3b82f6', width=2, dash='dot')
        ))

        fig.add_trace(go.Scatter(
            x=prices, y=profits, mode='lines+markers', name='Profit',
            line=dict(color='#10b981', width=3)
        ))
        
        _, _, curr_prof = self.predict(input_data)
        fig.add_trace(go.Scatter(
            x=[base_price], y=[curr_prof], mode='markers', name='Current Selection',
            marker=dict(color='white', size=12, line=dict(width=2, color='red'))
        ))
