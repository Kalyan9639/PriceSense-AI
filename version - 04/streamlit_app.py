...
with col_rec:
        ...
        with st.spinner("AI is analyzing market trends..."):
            try:
                prompt = (
                    ...
                    f"Provide exactly 3 concise, professional bullet points on how to improve profit and market share. "
    ...
    if st.session_state.get("ai_advice"):
        advice_lines = [
            line.strip() for line in st.session_state.ai_advice.strip().splitlines()
            if line.strip()
        ]
        bullets_html = "".join(
            f'<li style="margin-bottom: 12px; line-height: 1.6; color: #e6edf3;">{ln.lstrip("•\-123456789. ")}</li>'
            for ln in advice_lines
        )
        st.markdown(f"""
    <div style="background: #1f242d; border: 1px solid #388bfd; border-radius: 12px; padding: 24px; margin-top: 12px;">
